...
from ml_model import predict_tip  # Add this import
...
with tab1:
    ...
    if st.button('Predict Tip'):
        ...
        importance = pd.DataFrame({
            'Feature': ['Total Bill', 'Party Size', 'Gender', 'Smoker', 'Day', 'Time'],
...
with tab2:
    ...
    st.write('Upload a CSV file with columns: total_bill, size, sex, smoker, day, time')
    
    uploaded_file = st.file_uploader('Choose a CSV file', type='csv')
    if uploaded_file is not None:
        try:
            batch_data = pd.read_csv(uploaded_file)
            required_columns = ['total_bill', 'size', 'sex', 'smoker', 'day', 'time']
            ...
            if all(col in batch_data.columns for col in required_columns):
                ...
                st.error('CSV must contain columns: total_bill, size, sex, smoker, day, time')
